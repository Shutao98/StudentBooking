...
public class EditServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("edit debug0");
		...
		request.setAttribute("editView", student);		
		...
		//bookingadvisordb.studentbooking
			//if(student.validate()){
			if(true){	
				System.out.println("edit debug1");
				
				
				//Update edit data to database								
				stmt.executeUpdate("UPDATE studentbooking SET studentname='" + studentName
						+ "',email='" + email + "', phone='" + phone + "', campus='" + campus + "', school='"
						+ school + "', advisortype='" + advisorType + "', week='" + week + "', weekday='"
						+ weekDay + "', batch='" + batch + "' WHERE studentid='" + studentId + "';");
				
				System.out.println("edit debug6");
				//Dispatch to confirmation view			
				RequestDispatcher rd = request.getRequestDispatcher("/JSP/EditView.jsp");
				rd.forward(request, response);
			/*}else{
			...
			//Display error alert
			System.out.println("edit debug8");
